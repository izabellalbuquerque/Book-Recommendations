# Book-Recommendations

## Indicação de livros para a área de tecnologia

Neste repositório, você encontra recomendações de livros para diversas áreas da tecnologia, como desenvolvimento front-end, back-end, UX/UI, ciência de dados, segurança da informação, engenharia de software, criptografia e muito mais.

A proposta é reunir boas indicações para quem deseja aprofundar os estudos, aprender novos conceitos ou construir uma base teórica sólida para seguir na carreira.

As sugestões estão organizadas por temas e incluem autores renomados e títulos que podem ser encontrados com facilidade em livrarias ou bibliotecas digitais. É uma curadoria pensada para quem está estudando, começando na área ou buscando ampliar seus conhecimentos técnicos.

📌**Os livros destacados em negrito são obras amplamente reconhecidas e considerados referências em suas respectivas áreas.**

⭐ **Não se esqueça de marcar este repositório com uma estrela para apoiar e acompanhar futuras atualizações!**  
💡 **Dica: Use Ctrl + F para buscar rapidamente o que precisa.**

## Sumário

- [Algoritmos](#algoritmos)
- [Arquitetura](#arquitetura)
- [Automação](#automação)
- [Back-end](#back-end)
- [C/C++](#cc)
- [C#](#c)
- [Carreira, Soft Skills e Liderança](#carreira-soft-skills-e-liderança)
- [Ciência de Dados, Estatística e IA](#ciência-de-dados-estatística-e-ia)
- [Compiladores](#compiladores)
- [Computação](#computação)
- [Design](#design)
- [Front-end](#front-end)
- [Games](#games)
- [Git](#git)
- [Go](#go)
- [Hardware](#hardware)
- [Inteligência Artificial](#inteligência-artificial)
- [Internet das Coisas (IoT)](#internet-das-coisas-iot)
- [Java](#java)
- [JavaScript](#javascript)
- [Linux](#linux)
- [Melhorando Skills](#melhorando-skills)
- [Mobile](#mobile)
- [PHP](#php)
- [Python](#python)
- [Redes](#redes)
- [Requisitos de Software](#requisitos-de-software)
- [Ruby](#ruby)
- [Scrum](#scrum)
- [Segurança da Informação](#segurança-da-informação)
- [Shell Script](#shell-script)
- [Sistemas](#sistemas)
- [Software](#software)

## Algoritmos
- Algoritmos - Robert Sedgewick, Kevin Wayne
- Algoritmos: Lógica para Desenvolvimento de Computadores – José Augusto N. G. Manzano, Jayr Figueiredo de Oliveira
- **Algoritmos: Teoria e Prática** - Thomas H. Cormen, Charles E. Leiserson, Ronald L. Rivest, Clifford Stein
- Algoritmos para Viver: A Ciência da Computação nas Decisões Humanas - Brian Christian, Tom Griffiths
- **Entendendo Algoritmos: Um Guia Ilustrado Para Programadores e Outros Curiosos** - Aditya Y. Bhargava
- Estrutura de Dados, Algoritmos, Análise da Complexidade e Implementações em Java e C/C++ - Ana Fernanda Gomes Ascencio, Edilene Aparecida Veneruchi de Campos
- Estruturas de Dados e Algoritmos com JavaScript - Loiane Groner
- Fundamentos da Programação de Computadores: Algoritmos, Pascal, C, C++ e Java - Ana Fernanda Gomes Ascencio, Edilene Aparecida Veneruchi de Campos
- **A Arte da Programação de Computadores** - Donald E. Knuth

## Arquitetura
- **Arquitetura Limpa: O Guia do Artesão para Estrutura e Design de Software** - Robert C. Martin
- Caixa de Ferramentas DevOps: Um Guia para Construção, Administração e Arquitetura de Sistemas Modernos - John Willis, Jez Humble, Gene Kim
- **Organização Estruturada de Computadores** - Andrew S. Tanenbaum, Todd Austin
- UML 2: Uma Abordagem Prática - Gilleanes Guedes
- **Software Architecture in Practice** - Len Bass
- **Continuous Delivery** - Jez Humble, David Farley

## Automação
- Descomplicando o Docker - José Vitor S. B. Ferreira, Rafael Gomes
- DevOps na Prática: Entrega de Software Confiável e Automatizada - Danilo Sato
- DevOps Nativo de Nuvem com Kubernetes - John Arundel, Justin Domingus
- Kubernetes: Tudo Sobre Orquestração de Contêineres - John Arundel, Justin Domingus

## Back-end
- Aplicações Java para a Web com JSF e JPA - Rafael Sakurai
- Aplicações Web Real-Time com Node.js - Caio Ribeiro Pereira
- ASP.NET Core MVC: Aplicações Modernas em Conjunto com o Entity Framework - Everton Coimbra de Araújo
- Desenvolvimento Web com PHP e MySQL - Juliano Niederauer
- Dominando JavaScript com jQuery - Cody Lindley
- MySQL: Comece com o Principal Banco de Dados Open Source do Mercado - Vinicius M. Carvalho
- NoSQL: Como Armazenar os Dados de uma Aplicação Moderna - Adriano Almeida
- O Canivete Suíço do Desenvolvedor Node - Caio Ribeiro Pereira
- PL/SQL: Domine a Linguagem do Banco de Dados Oracle - Benjamin A. Rosenzweig, Elena Silvestrova
- PostgreSQL: Banco de Dados para Aplicações Web Modernas - Jayme F. de Oliveira Jr.
- SQL: Uma Abordagem para Bancos de Dados Oracle - Benjamin A. Rosenzweig, Elena Silvestrova

## C/C++
- C: Como Programar - Paul Deitel, Harvey Deitel
- C: Completo e Total - Herbert Schildt
- C++ Concurrency in Action - Anthony Williams
- C++20 for Programmers: An Objects-Natural Approach - Paul Deitel, Harvey Deitel
- Effective Modern C++ - Scott Meyers
- Embedded C - Michael J. Pont
- Introdução à Programação em C: Os Primeiros Passos de um Desenvolvedor - Gustavo Furtado de Oliveira Alves
- Introdução à Programação em C++ - Y. Daniel Liang
- Orientação a Objetos e SOLID para Ninjas: Projetando Classes Flexíveis - Mauricio Aniche
- Programming Embedded Systems in C and C++ - Michael Bar
- Test-Driven Development: Teste e Design no Mundo Real - Kent Beck

## C#
- ASP.NET Core MVC: Aplicações Modernas em Conjunto com o Entity Framework - Everton Coimbra de Araújo
- Test-Driven Development: Teste e Design no Mundo Real com DotNet - Mauricio Aniche
- **Use a Cabeça C#** - Andrew Stellman, Jennifer Greene

## Carreira, Soft Skills e Liderança
- Acelerando: Construindo Organizações de Tecnologia de Alta Performance - Nicole Forsgren, Jez Humble, Gene Kim
- **O Mítico Homem-Mês** - Frederick P. Brooks Jr.
- O Projeto Fênix - Gene Kim, Kevin Behr, George Spafford
- Soft Skills: O Manual da Vida do Desenvolvedor de Software - John Sonmez
- **Peopleware: Productive Projects and Teams** - Tom DeMarco, Timothy Lister

## Ciência de Dados, Estatística e IA
- **Aprendizado de Máquina na Prática com Scikit-Learn, Keras e TensorFlow** - Aurélien Géron
- **Aprendizado Profundo** - Ian Goodfellow, Yoshua Bengio, Aaron Courville
- Ciência de Dados do Zero - Joel Grus
- Inteligência Artificial: Um Guia para Sistemas Inteligentes - Michael Negnevitsky
- Introdução ao Aprendizado Estatístico - Gareth James, Daniela Witten, Trevor Hastie, Robert Tibshirani
- Os Elementos do Aprendizado Estatístico - Trevor Hastie, Robert Tibshirani, Jerome Friedman
- **Python para Análise de Dados** - Wes McKinney
- Reconhecimento de Padrões e Aprendizado de Máquina - Christopher Bishop
- **Estatística Prática para Ciência de Dados** - Peter Bruce, Andrew Bruce, Peter Gedeck
- **Padrões de Projetos de Machine Learning** - Valliappa Lakshmanan, Sara Robinson, Michael Munn
- **Think Stats: Probabilidade e Estatística para Cientistas de Dados** - Allen B. Downey

## Compiladores
- Compiladores - Alfred V. Aho, Monica S. Lam, Ravi Sethi, Jeffrey D. Ullman
- Compiladores - Princípios e Prática – Kenneth C. Louden

## Computação
- Código: A Linguagem Oculta do Hardware e Software de Computador - Charles Petzold
- **Estrutura e Interpretação de Programas de Computador** - Harold Abelson, Gerald Jay Sussman
- **Computational Thinking for the Modern Problem Solver** - David D. Riley, Kenny A. Hunt

## Design
- Introdução e Boas Práticas em UX Design - Fabricio Teixeira
- **Padrões de Projeto: Soluções Reutilizáveis de Software Orientado a Objetos** - Erich Gamma, Richard Helm, Ralph Johnson, John Vlissides
- Não Me Faça Pensar - Steve Krug
- O Design do Dia a Dia - Don Norman

## Front-end
- **HTML e CSS: Projete e Construa Websites** - Jon Duckett
- **JavaScript e JQuery: Desenvolvimento de Interfaces Web Interativas** - Jon Duckett
- Design Responsivo: Trabalhando com HTML5, CSS3 e JavaScript - Ben Frain

## Games
- Desenvolvimento de Jogos 3D - David H. Eberly

## Git
- Controlando Versões com Git e GitHub - Alexandre Aquiles, Rodrigo Ferreira

## Go
- Programando em Go: Crie Aplicações com a Linguagem do Google - Caleb Doxsey

## Hardware
- Analog and Digital Circuits for Electronic Control System Applications: Using the TI MSP430 Microcontroller - Jerry Luecke
- Arduino Básico - Michael McRoberts
- Arduino Descomplicado - Claudio Luis Vieira Oliveira, Humberto Augusto Piovesana Zanetti
- Getting Started with Arduino - Massimo Banzi
- Getting Started with the MSP430 Launchpad - Adrian Fernandez, Dung Dang
- Introduction to Embedded Systems: Using Microcontrollers and the MSP430 - Manuel Jiménez, Rogelio Palomera, Isidoro Couvertier
- Organização Estruturada de Computadores - Andrew S. Tanenbaum
- PIC Microcontroller Project Book - John Lovine
- Sensors and Transducers - Ian Sinclair

## Inteligência Artificial
- Inteligência Artificial: Uma Abordagem Moderna - Stuart Russell, Peter Norvig

## Internet das Coisas (IoT)
- **Building the Internet of Things** - Maciej Kranz
- **Internet das Coisas: Tecnologias, Aplicações e Desafios** - Samuel Greengard

## Java
- Componentes Reutilizáveis em Java com Reflexões e Anotações - Eduardo Guerra
- Design Patterns com Java: Projeto Orientado a Objetos Guiado por Padrões - Eduardo Guerra
- Java 8 Prático: Lambdas, Streams e os Novos Recursos da Linguagem - Paulo Silveira, Guilherme Silveira
- Java: A Beginner's Guide - Herbert Schildt
- Java Como Programar - Paul Deitel, Harvey Deitel
- Java SE 8 Programmer I: O Guia para Sua Certificação Oracle Certified Associate - Guilherme Silveira, Mário Amaral
- MundoJ: Java Efetivo - Joshua Bloch
- Spring MVC: Domine o Principal Framework Web Java - Alberto Souza
- **Use a Cabeça! Java** - Kathy Sierra, Bert Bates

## JavaScript
- Cangaceiro JavaScript: Uma Aventura no Sertão da Programação - Flávio Almeida
- JavaScript: ECMAScript 6 - Entre de Cabeça no Futuro do JavaScript - Diego Martins de Pinho
- JavaScript: O Guia Definitivo - David Flanagan
- JavaScript and jQuery: Interactive Front-End Web Development - Jon Duckett
- JavaScript de Alto Desempenho - Nicholas C. Zakas
- **JavaScript Eloquente** - Marijn Haverbeke
- JavaScript for Beginners - Kirupa Chinnathambi
- Lógica de Programação: Crie Seus Primeiros Programas Usando JavaScript e HTML - Adriano Almeida
- MEAN: Full Stack JavaScript para Aplicações Web com MongoDB, Express, Angular e Node - Flávio Almeida
- Primeiros Passos com React - Stoyan Stefanov
- **Você Não Conhece JavaScript** - Kyle Simpson

## Linux
- Linux: A Bíblia - Christopher Negus

## Melhorando Skills
- A Startup Enxuta - Eric Ries
- **Código Limpo** - Robert C. Martin
- Código: Um Manual Prático de Construção de Software - Steve McConnell
- Como Ser um Programador Melhor - Pete Goodliffe
- Cracking the Coding Interview - Gayle Laakmann McDowell
- eXtreme Programming: Práticas para o Dia-a-Dia no Desenvolvimento Ágil de Software - Vinicius Manhães Teles
- Guia do Mestre Programador - J. B. Rainsberger
- LEAN ENTERPRISE: Como Empresas de Alta Performance Inovam em Escala - Jez Humble, Joanne Molesky, Barry O’Reilly
- O Programador Apaixonado: Construindo uma Carreira Notável em Desenvolvimento de Software - Chad Fowler
- **O Programador Pragmático** - Andrew Hunt, David Thomas
- **Refatoração: Aperfeiçoando o Design de Códigos Existentes** - Martin Fowler
- SEO Prático: Seu Site na Primeira Página das Buscas - Adriano Almeida
- **Domain-Driven Design** - Eric Evans

## Mobile
- jQuery Mobile: Desenvolva Interfaces para Múltiplos Dispositivos - Maximiliano Firtman

## PHP
- Análise de Vulnerabilidades em Sistemas Web com PHP - Juliano Niederauer
- Core PHP Programming: Using PHP to Build Dynamic Web Sites - Leon Atkinson
- Laravel: Up and Running: A Framework for Building Modern PHP Apps - Matt Stauffer
- Learn PHP 7: Object Oriented Modular Programming using HTML5, CSS3, JavaScript, XML, JSON, and MySQL - Steve Prettyman
- PHP: Programando com Orientação a Objetos - Pablo Dall’Oglio
- PHP and MySQL Web Development: Master the Concepts of PHP: A Step By Step Process - Luke Welling, Laura Thomson
- PHP: Learn PHP in 24 Hours or Less - A Beginner's Guide To Learning PHP Programming Now - Robert Dwight
- Practical PHP 7, MySQL 8, and MariaDB Website Databases: A Simplified Approach to Developing Database-Driven Websites - Adrian W. West

## Python
- Automatize Tarefas Maçantes com Python - Al Sweigart
- Curso Intensivo de Python - Eric Matthes
- Data Science do Zero: Primeiras Regras com o Python - Joel Grus
- Django: The Easy Way: A Step-by-Step Guide on Building Django Websites - Samuli Natri
- Flask Web Development: Developing Web Applications with Python - Miguel Grinberg
- Pense em Python - Allen B. Downey
- Python All-In-One for Dummies - John Shovic, Alan Simpson
- Python: Escreva Seus Primeiros Programas - Luiz Eduardo Borges
- **Python para Análise de Dados** - Wes McKinney
- Python para Desenvolvedores - Luiz Eduardo Borges
- Python para Todos: Explorando Dados com Python - Charles Severance
- Test-Driven Development with Python: Obey the Testing Goat: Using Django, Selenium, and JavaScript - Harry J.W. Percival
- Web Scraping with Python: Collecting More Data from the Modern Web - Ryan Mitchell

## Redes
- Comunicação de Dados e Redes de Computadores - Behrouz A. Forouzan
- Desconstruindo a Web: As Tecnologias por Trás de uma Requisição - Willian Molinari
- Redes de Computadores: Uma Abordagem Top-Down - James F. Kurose, Keith W. Ross

## Requisitos de Software
- Agile Software Requirements: Lean Requirements Practices for Teams, Programs, and the Enterprise - Dean Leffingwell
- User Stories Applied - Mike Cohn

## Ruby
- Ruby: Aprenda a Programar na Linguagem Mais Divertida - Lucas Souza

## Scrum
- Scrum: Gestão Ágil para Projetos de Sucesso - Rafael Sabbagh

## Segurança da Informação
- Cibersegurança e Ciberguerra - P.W. Singer, Allan Friedman
- **Engenharia de Segurança** - Ross Anderson
- Hacking Para Leigos - Kevin Beaver
- **Hacking: A Arte de Exploração** - Jon Erickson
- **Segurança de Computadores: Princípios e Práticas** - William Stallings, Lawrie Brown
- **Criptografia e Segurança de Redes** - William Stallings
- **The Web Application Hacker’s Handbook** - Dafydd Stuttard, Marcus Pinto

## Shell Script
- Pequeno Manual do Programador Bash - Blau Araújo
- Shell Script Profissional - Aurélio Marinho Jargas

## Sistemas
- Embedded Systems Design Using the TI MSP430 Series - Chris Nagy
- MSP430-Based Robot Applications: A Guide to Developing Embedded Systems - Dan Harres
- **Sistemas Operacionais: Conceitos e Design** - Abraham Silberschatz, Peter B. Galvin

## Software
- Agile: Desenvolvimento de Software com Entregas Frequentes e Foco no Valor de Negócio - André Faria Gomes
- Desenvolvimento Ágil Limpo - Robert C. Martin
- **Engenharia de Software** - Ian Sommerville
